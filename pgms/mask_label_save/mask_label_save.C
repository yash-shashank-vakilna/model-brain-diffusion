...
static vfimage extract_frm_segim(const vfimage& dwimage, const limage& seg_image,
										const unsigned int& tissue_type)
//! removes everything in dwimage except where segimage==tissue type 
{
	vfimage cropped_image; cropped_image = dwimage;						//allocates images
	uvec3 s, ex = seg_image.extent(), ex_dwi = dwimage[0].extent();
	if ((ex.x != ex_dwi.x) || (ex.y != ex_dwi.y) || (ex.z != ex_dwi.z))
	{
		 throw optException("dwi and seg image extent mismatch"); 
	}
	fwIterator fi(ex,0); 
	size_t no_im = cropped_image.size();
	while(fi(s))
	{
		if(seg_image(s) != static_cast<long int>(tissue_type))			//matching tissue types //1= CSF, 2=GM, 3=WM
		{			
			for (unsigned int i = 0; i < no_im; i++)				
			{
				cropped_image[i](s)=0;										//removing tissues which are not required
			}
		}
	}
...
{	limage dst(ex); dst = 0; dst.copyAttributes(src[0].at);				// allocate destination image
	...
	if (!rname.empty()) { fimage ref; ref.read(rname.c_str());			// add T2 image if specified 
 ...
 /********Pre-built dwimage from Brian 3.1 ends************************/
 
static void crop_using_mask(limage& src, const bimage& mask)
//! use bimage mask to crop src
{
...
//! extract first serial numbers from inp string
{
		...
		std::string fname(argv[2],len-16,6);									//clus_fname = filename of the labeled image
		return fname;
}
...
{	int status = 0;
	try {	
		FILE *inf = nullptr, *outf = nullptr;							//ip: dwi_image op:labeled segmented image
		vfimage dwimage; 
		option opts[] = { };			
		std::string serial = getSerialID(argc, argv);					//serial number based on inp fname
		serial.append("_t2_pl.v");
		cmdline c(argc, argv, 0, opts); c.parse(&inf, &outf);			//Parsing cmdline input
		readImages(dwimage,inf);
		printf("Image loaded\n");
		new_dwimage dwiI(dwimage,0);
		limage segmented_image = dwiI.segmentCompartments
									(serial);							//segment dwi image
		
		unsigned int b0_ind = dwimage.size()-1;
		bimage mask;
		mask = getmask(dwimage[b0_ind]);								//compute mask 
		crop_using_mask(segmented_image, mask);							//crop dwImage
