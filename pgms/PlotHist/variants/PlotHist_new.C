/*
 *
 * bdprop.C: Estimate voxel-wise properties in diffusion-weighted MR image series
...
static fimage PlotHist(const vfimage& fim, const int no_bins)
//Plots 2-D histogram based on no. of bins
//outputs in fimage
//only populateHist dim dependent
{	fimage hist(no_bins,no_bins,1); hist=0.0f; 
    ...
    for(int i=0;i<no_dim;i++)	//populating max,min
    	{
    	    ...
    	    max_ele[i] = std::numeric_limits<float>::min();
	    min_ele[i] = std::numeric_limits<float>::max();
    	    uvec3 ex = fim[i].extent(),s; //check fwIterator once
    	    fwIterator fi(ex,0);
    	    while (fi(s)) { const float v = fim[i](s);
...
{	int status = 0;
	try {	FILE *inf = nullptr, *outf = nullptr;
		option opts[] = { };
		cmdline c(argc, argv, 0, opts); c.parse(&inf, &outf);
